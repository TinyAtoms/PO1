...
class Plantgroup(models.Model):
    ...
    kc = models.FloatField(
        help_text="dimmensionless plant coefficient, usually around 1")
    area = models.FloatField(help_text="area in square meters")
    water_flowrate = models.FloatField(help_text="flowrate in l/min.")
    water_t1 = models.TimeField(default=datetime.time(12, 0, 0))
    last_irrigation = models.FloatField(null=True, blank=True, help_text="How much liter water was used in the last irrigation")
...
def get_pan_coeff():
    ...
    if len(pan_data) < 10:
        return 1
    FAO_data = [FAO_data[i[0]] for i in pan_data]
    pan_data = [i[1] for i in pan_data]
    model = sm.OLS(FAO_data, pan_data).fit()
